...
{
    public class TutorialAutoBattleRewardBackHome : TutorialController
    {
        //private JustButton btnHome;
        private JustButton btnLVUP;
        public TutorialAutoBattleRewardBackHome(int pId, bool pIsToolTips) : base(pId, pIsToolTips)
        {
        }
        ...
        public override IEnumerator IEProcess()
        {
            yield return new WaitUntil(() => (MainGamePanel != null
                                                          && (MainGamePanel.WinPanel.IsActiveAndEnabled() || MainGamePanel.LosePanel.IsActiveAndEnabled())));
	...
	*/
            ...
            if (MainGamePanel.WinPanel.IsActiveAndEnabled()) btnLVUP = MainGamePanel.WinPanel.btnHome;
            //else btnHome = MainGamePanel.LosePanel.btnHome;
            TutorialsManager.InputMasker.FocusToTargetImmediately(btnLVUP.rectTransform);
            //s = "Now go back to the main screen.I will guide you on how to claim Idle Reward";
            //s = Localization.Get(Localization.ID.TUTORIAL_92);
            s = "";
            board = MainGamePanel.ShowNotificationBoard(btnLVUP.rectTransform, s, Alignment.Top, new Vector2(144f, -515f), true, false);
            board.transform.SetParent(TutorialsManager.transform);
