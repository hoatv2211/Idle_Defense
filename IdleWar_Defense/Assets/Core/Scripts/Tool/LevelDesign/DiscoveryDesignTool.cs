...
public class DiscoveryDesignTool : MonoBehaviour
{
    ...
    {
        return Application.dataPath + "/Core/Resources/LevelDesign/Discovery";
    }
    ...
    private void LoadAllMission()
    {
        ...
        System.IO.DirectoryInfo dir = new System.IO.DirectoryInfo(Application.dataPath + "/Core/Resources/LevelDesign/Discovery/");
        var file = dir.GetFiles();
        var count = file.Length / 2;//vì chứa cả file meta
        for (int i = 0; i < count; i++)
        {
            int index = i;
            var btnMission = GameObject.Instantiate(btnMissionPrefab, transformBtnMissionsPool);
            btnMission.onClick.AddListener(() => { ChoiceMission(index); });
            ...
            var missionInfo =
                JsonUtility.FromJson<MissionInfo>(
                    ReadString(GetFolderMap() + "/mission_discovery_" + (index + 1) + ".json"));
            int totalPower = 0;
            if (missionInfo != null)
            {
    ...
    private void ChoiceMission()
    {
        ...
        if (File.Exists(GetFolderMap() + "/mission_discovery_" + (currentMissionIndex + 1) + ".json"))
        {
            var missionInfo = JsonUtility.FromJson<MissionInfo>(ReadString(GetFolderMap() + "/mission_discovery_" + (currentMissionIndex + 1) + ".json"));
            if (missionInfo != null)
            {
    ...
    private void BtnSave_Pressed()
    {
        ...
        WriteString(GetFolderMap() + "/mission_discovery_" + (currentMissionIndex + 1) + ".json", json);
