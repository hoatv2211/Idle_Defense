...
{
    [AddComponentMenu("Layout/Good Grid Layout Group", 153)]
    /// <summary>
    ...
    /// </remarks>
    public class GoodGridLayoutGroup : LayoutGroup
    {
        /// <summary>
        ...
        {
            var lastSpacing = GetLastSpacing();
            ...
            SetLayoutInputForAxis(
                padding.horizontal + (cellSize.x + lastSpacing.x) * minColumns - lastSpacing.x,
                padding.horizontal + (cellSize.x + lastSpacing.x) * preferredColumns - lastSpacing.x,
                -1, 0);
        }
        ...
        public override void CalculateLayoutInputVertical()
        {
            ...
            else
            {
                ...
                int cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - padding.horizontal + lastSpacing.x + 0.001f) / (cellSize.x + lastSpacing.x)));
                minRows = Mathf.CeilToInt(rectChildren.Count / (float)cellCountX);
            }

            float minSpace = padding.vertical + (cellSize.y + lastSpacing.y) * minRows - lastSpacing.y;
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            {
                if (cellSize.x + lastSpacing.x <= 0)
                    cellCountX = int.MaxValue;
                else
                    cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - padding.horizontal + lastSpacing.x + 0.001f) / (cellSize.x + lastSpacing.x)));

                if (cellSize.y + lastSpacing.y <= 0)
                    cellCountY = int.MaxValue;
                else
                    cellCountY = Mathf.Max(1, Mathf.FloorToInt((height - padding.vertical + lastSpacing.y + 0.001f) / (cellSize.y + lastSpacing.y)));
            }
            ...
            Vector2 requiredSpace = new Vector2(
                actualCellCountX * cellSize.x + (actualCellCountX - 1) * lastSpacing.x,
                actualCellCountY * cellSize.y + (actualCellCountY - 1) * lastSpacing.y
            );
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (cellSize[0] + lastSpacing[0]) * positionX, cellSize[0]);
                SetChildAlongAxis(rectChildren[i], 1, startOffset.y + (cellSize[1] + lastSpacing[1]) * positionY, cellSize[1]);
            }
        }

        private Vector2 GetLastSpacing()
        {
            var lastSpacing = new Vector2(spacing.x, spacing.y);
            
            //tính lại spacing cho nó explan all
            var size = rectTransform.rect.size;
            int count = (int) ((size.x - padding.horizontal) / cellSize.x);
            //nếu số child trong list đủ một dòng thì phải giãn lại spacing
            if (rectChildren.Count >= count)
            {
                var delta = ((size.x - padding.horizontal) - count * cellSize.x) / ((float) count - 1f);
                lastSpacing = new Vector2(delta, spacing.y);
            }
