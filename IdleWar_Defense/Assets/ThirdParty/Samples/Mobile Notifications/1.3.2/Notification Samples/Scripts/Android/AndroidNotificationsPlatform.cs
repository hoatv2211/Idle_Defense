...
namespace NotificationSamples.Android
{
        ...
        IDisposable
    {
        ...
        public void ScheduleNotification(AndroidGameNotification gameNotification)
        {
            ...
            AndroidNotification no = new AndroidNotification();
            no.RepeatInterval = new TimeSpan(1, 0, 0, 0);

            if (gameNotification.Id.HasValue)
            {
                var notificationStatus = AndroidNotificationCenter.CheckScheduledNotificationStatus(gameNotification.Id.Value);
                //    UnityEngine.Debug.Log("nono " + notificationStatus.ToString() + "_" + gameNotification.Body);
