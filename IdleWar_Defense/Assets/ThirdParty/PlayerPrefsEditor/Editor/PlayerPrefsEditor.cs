...
public class PlayerPrefsEditor : EditorWindow
{
	...
	void ExportData()
	{
		string data = JsonUtility.ToJson(deserializedPlayerPrefs);
		//foreach (PlayerPrefPair item in deserializedPlayerPrefs)
		//{
		//	Debug.LogError(item.ToString());
		//}
		string path = EditorUtility.OpenFolderPanel("Hello", "", "");
		...
		SerializeObject<List<PlayerPrefPair>>(deserializedPlayerPrefs, path);
		Debug.LogError("Save done: " + path);
	...
	void ImportData()
	{
		string file = EditorUtility.OpenFilePanel("Hello", "", "txt");
		List<PlayerPrefPair> importValues = (List<PlayerPrefPair>)DeSerializeObject<List<PlayerPrefPair>>(file);
		foreach (PlayerPrefPair item in importValues)
		{
			//Debug.LogError(item.ToString());
...
public class PlayerPrefPair
{
	...
	{
		return Key + "_" + Value + "_" + Value.GetType().ToString();
	}
}
